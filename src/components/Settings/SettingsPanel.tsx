...
export function SettingsPanel() {
  ...
  const { setProjects, setRepositories } = useProjectStore()
  ...
  const handleRefreshState = async () => {
    ...
    setIsReloading(true)
    try {
      const newState = await window.electronAPI.reloadState()
      // Update the store with the reloaded state
      if (newState) {
        setProjects(newState.projects || [])
        setRepositories(newState.repositories || [])
      }
    } finally {
            ...
            <div className="space-y-4">
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
                  Reload the application state from the state.json file. This is useful if you've made manual changes to the file or if the UI is out of sync.
                </p>
                <button
